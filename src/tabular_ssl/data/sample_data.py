...
def load_credit_card_sample(
    ...
    try:
        ...
        print(f"✅ Prepared {len(df)} transaction sequences for {metadata['n_users']} users")
...
def _preprocess_transactions(
    ...
    valid_users = user_counts[
        (user_counts >= min_transactions) & 
        (user_counts <= max_transactions)
    ].index
        ...
        if df[col].dtype == 'object' or df[col].dtype.name == 'category':
            ...
            if df[col].nunique() < len(df) * 0.5:
                categorical_features.append({
                    ...
                    'categories': df[col].unique().tolist()[:10]  # First 10 for display
                })
